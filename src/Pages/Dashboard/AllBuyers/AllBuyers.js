...
import DeleteModal from '../AllSeller/DeleteModal/DeleteModal';
...
const AllBuyers = () => {
    ...
    const { data: setCurrentUser, isLoading, refetch } = useQuery({
        ...
        queryFn: async () => {
            const res = await fetch(`https://assignment-12-server-rosy.vercel.app/users/buyers`)
    ...
    return (
        <div>
            {setCurrentUser.length > 0 ? <>   <h1 className='text-3xl pb-5 font-bold'>Manage All Buyers </h1>
